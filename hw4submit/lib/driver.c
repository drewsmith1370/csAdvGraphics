...
int main() {
   ...
   VK_CHECK(CreateLogicalDevice(&device,features,NULL,0,NULL,1,VK_QUEUE_GRAPHICS_BIT|VK_QUEUE_COMPUTE_BIT));
   ...
   // Create the buffer
   VK_CHECK(DeviceCreateBuffer(device, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,&buf,sizeof(float[1024]),data));
   printf("Created buffer with 4096 bytes on %s\n",device.properties.deviceName);
   // Destroy the buffer
   DestroyBuffer(&buf);
   DestroyDevice(device);
   printf("Resources freed. Returning\n");
