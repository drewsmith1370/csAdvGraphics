...
void display()
{
   ...
   UniformBufferData(frame, &ubo,sizeof(ubo));
   // Clear fences
   ClearFences();
   ...
   QueueForPresentation(window,imageIndex,framebufferResized,&frame);
}
...
int main()
{
   ...
   CreateDevice(window, &physicalDevice, &device);
   //  Create the swapchain, image and framebuffers
   CreateSwapChain(1,window);
   //  Load texture
   CreateTextureImage("pi.bmp", device, physicalDevice);
